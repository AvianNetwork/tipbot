...
export const wealth = async (message: Discord.Message) => {
	...
	const wealthRequest = await fetch(`${config.project.explorer}ext/getdistribution`).catch(async (error) => {
...
export const qr = async (message: Discord.Message) => {
	// Get the date and parse the address
	const date = new Date().toUTCString().replace(`,`, ` `);
	...
	if (!config.coin.address.test(address)) {
		return helper.sendErrorMessage(
			message,
			`**  ${config.coin.name} (${config.coin.symbol}) QR Code  **`,

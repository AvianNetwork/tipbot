	...
	setInterval(async () => {
		...
		infoBots.forEach(async (bot) => {
			...
			if (!miningInfoData || !miningInfoData[1]) {
				await log(
					`Error fetching the mining info: ${miningInfoData[0] || miningInfoData || `Unknown error`}`,
				);
				return;
			}
			...
			switch (bot.function) {
				...
				case `MinotaurX_hashrate`:
					const hashrate_MinotaurX = Number(miningInfoData[1][`networkhashps_minotaurx`] / 1000000).toFixed(
						3,
					);
...
process.on(
	`uncaughtException`,
	async (error, origin) => await log(`uncaughtException: ${error}\nOrigin: ${origin}`),
);

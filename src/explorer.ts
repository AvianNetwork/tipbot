...
export const supply = async (message: Discord.Message) => {
    ...
    const supplyData: any = await (await fetch(`${config.project.explorerurl}ext/getmoneysupply`)).text().catch((error) => {
...
export const wealth = async (message: Discord.Message) => {
    const date = new Date().toUTCString().replace(`,`, ` `);
    const wealthData: any = await (await fetch(`${config.project.explorerurl}ext/getdistribution`)).json().catch((error) => {
        main.log(`Error fetching the supply: ${error}`);
        return undefined;
    });
